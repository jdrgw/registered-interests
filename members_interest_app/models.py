...
class RegisteredInterest(models.Model):
    ...
    # fields from api
    api_id = models.CharField(max_length=255, unique=False, null=False)  # api_id non-unique
    unique_api_generated_id = models.CharField(max_length=255, unique=True, null=False)  # api_id non-unique – create from count of api_id-member_id pairing
    ...
    date_last_amended = models.DateTimeField(null=True)  # sometimes absent in API data (25%)
    date_deleted = models.DateTimeField(null=True, blank=True)  # Allowing null for non-deleted entries
    ...
    interest_amount = models.DecimalField(null=True, blank=True, decimal_places=2, max_digits=12)
    CURRENCY_CHOICES = (
        ("GBP", "Pound Sterline"),
        ("USD", "US Dollar"),
        ("EUR", "Euro"),
    ...
    interest_currency = models.CharField(max_length=50, null=True, blank=True, choices=CURRENCY_CHOICES)
    ...
    contains_time_period = models.BooleanField(null=True, blank=True)  # some entries contain values per <event>/<time period>, this flag indicates quality issues
    ...
    payer_type = models.CharField(max_length=255, null=True, blank=True, choices=PAYER_TYPE_CHOICES)
