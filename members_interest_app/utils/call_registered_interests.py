...
# WRITE FUNCTION
from members_interest_app.models import MemberOfParliament  # noqa: E402

REGISTERED_INTEREST_API = (
...
def call_api_and_save_data(file_path=None):
    ...
    member_api_ids = (
        MemberOfParliament.objects.filter(api_id__isnull=False)
        .order_by("api_id")
            ...
            for i in range(0, len(member_api_ids), BATCH_SIZE):
                ...
                for index in batch:
                    ...
                    for attempt in range(1, ATTEMPTS + 1):
                        try:
                            print(
                                f"Attempting to fetch data for api_id: {index}, Attempt: {attempt}"
                            )
                            ...
                            if response.status_code == 200:
                                ...
                                print(
                                    f"Server error ({response.status_code}) for api_id: {index}"
                                )
                            elif (
                                ...
                                print(
                                    f"Rate-limited for api_id: {index}, retrying after {wait_time} seconds..."
                                )
                            else:
                                # For non-retriable errors (like 404), no need to retry
                                print(
                                    f"Non-retriable error ({response.status_code}) for api_id: {index}"
                                )
                                break

                        except requests.exceptions.RequestException as e:
                            print(
                                f"Network error on attempt {attempt} for api_id {index}: {str(e)}"
                            )
                            traceback.print_exc()
                    ...
                    if not success:
                        print(
                            f"Failed to retrieve data for api_id: {index} after {ATTEMPTS} attempts"
                        )
