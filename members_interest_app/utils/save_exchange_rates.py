...
def save_exchange_rates():
    ...
    """
    daily_aud_to_gbp = pd.read_csv(
        "data/exchange_rates/static_bank_of_england_daily_rates/XUDLADS  Bank of England  Database-daily-australian-dollar-to-sterling-fx-since-09.csv"
    )
    daily_eur_to_gbp = pd.read_csv(
        "data/exchange_rates/static_bank_of_england_daily_rates/XUDLERS  Bank of England  Database-daily-euro-to-sterling-fx-since-09.csv"
    )
    daily_gbp_to_usd = pd.read_csv(
        "data/exchange_rates/static_bank_of_england_daily_rates/XUDLGBD  Bank of England  Database-daily-sterling-to-usd-fx-since-09.csv"
    )
    ...
    daily_aud_to_gbp["gbp_aud_daily_spot_price"] = (
        1 / daily_aud_to_gbp["aud_gbp_daily_spot_price"]
    )
    daily_eur_to_gbp["gbp_eur_daily_spot_price"] = (
        1 / daily_eur_to_gbp["eur_gbp_daily_spot_price"]
    )

    currency_conversions = daily_gbp_to_usd.merge(
        daily_aud_to_gbp[["date", "gbp_aud_daily_spot_price"]], how="left", on="date"
    ).merge(
        daily_eur_to_gbp[["date", "gbp_eur_daily_spot_price"]], how="left", on="date"
    )
    ...
    currency_conversions_long = currency_conversions.melt(
        id_vars=["date"], var_name="currency", value_name="rate_to_gbp"
    )

    # make dates datetime objects - db field is DateField but django can parse datetime objects to dates
    currency_conversions_long["date"] = pd.to_datetime(
        currency_conversions_long["date"], format="%d %b %y"
    )
