...
EXTRACTABLE_AMOUNT_CATEGORIES = (
    CATEGORIES_WITH_EASILY_EXTRACTABLE_AMOUNTS + CATEGORIES_AMOUNTS_EXTRACTABLE_WITH_PROCESSING
)
...
def extract_member_id(links):
    ...
    for link in links:
        if not isinstance(link, dict) or "rel" not in link or "href" not in link:
            raise ValueError("Each link must be a dictionary with 'rel' and 'href' keys.")
...
def flatten_interests_to_df(dataset):
    ...
    child_interest_count = defaultdict(lambda: defaultdict(int))  # Count for child interests
        ...
        for category in entry["value"]:
            ...
            for interest in category["interests"]:
                ...
                for child_interest in interest.get("childInterests", []):
                    ...
                    child_interest_count_str = f"{member_id}-{interest_id}-{child_interest_id}-{child_count}"
    ...
    if df.empty:
        raise ValueError("DataFrame is empty. Something went wrong with flattening registered interest data.")
...
def remove_space_in_registration_numbers(df):
    ...
    registration_number_pattern = re.compile(r"(registration)\s+(\d+)", flags=re.IGNORECASE)
    
    df.loc[:, "edited_interest"] = df["interest"].apply(
        lambda x: registration_number_pattern.sub(lambda match: f"{match.group(1)}{match.group(2)}", x)
    )
    return df
...
def convert_abbreviated_numbers_to_numbers(df, column):
    ...
    if not mask.any():
        raise ValueError("Replacement text not found in the specified column. Something went wrong.")
...
def extract_max_amount_with_currency(dataframe, column_name):
        ...
        for entry in entry_list:
            ...
            try:
                ...
                for i, amount in enumerate(amounts):
                    ...
                    currency = currency_choice_map.get(currencies[i]) if i < len(currencies) else None
    ...
    dataframe[['split_amounts', 'split_currencies', 'max_amount', 'max_amount_currency']] = dataframe[column_name].apply(
        lambda x: pd.Series(process_entry(x))  # pd.Series enables population of multiple columns
    )
...
def mentions_debt_synonyms(df,col):
    ...
    df["contains_debt_synonym"] = df[col].str.contains(synonyms_pattern, regex=True, flags=re.IGNORECASE)
...
def mentions_time_period_or_preposition(df,col):
    ...
    time_periods_or_prepositions_pattern = r"\b(?:" + "|".join(time_periods_or_prepositions) + r")\b"

    df["contains_time_periods_and_prepositions"] = df[col].str.contains(time_periods_or_prepositions_pattern, regex=True, case=False)
...
def extract_payer_details(df):
    ...
    df["donor_name"] = df["interest"].apply(
        lambda x: re.search(donor_name_pattern, x).group(1) if re.search(donor_name_pattern, x) else None
    )
    df["donor_type"] = df["interest"].apply(
        lambda x: re.search(donor_type_pattern, x).group(1) if re.search(donor_type_pattern, x) else None
    )
    df["donor_address"] = df["interest"].apply(
        lambda x: re.search(donor_address_pattern, x).group(1) if re.search(donor_address_pattern, x) else None
    )
    df["companies_house_id"] = df["interest"].apply(
        lambda x: re.search(companies_house_id_pattern, x).group(1) if re.search(companies_house_id_pattern, x) else None
    )
...
def extract_mp_role_and_employer(df):
    ...
    employment_earnings_mask = (df["category_name"] == "1. Employment and earnings") & df["donor_name"].notnull()
    ...
    other_categories_mask = df["category_name"].isin(other_categories_with_employer_data)
    
    df.loc[employment_earnings_mask, "employer_name"] = df["donor_name"]
    
    df.loc[employment_earnings_mask, "mp_role"] = df["interest"].str.extract(pattern, expand=False)

    df.loc[other_categories_mask, "employer_name"] = df["interest"].str.split(",", n=1).str.get(1)

    df.loc[other_categories_mask, "mp_role"] = df["interest"].str.split(",", n=1).str.get(0)
...
def extract_family_member_info(df):
    ...
    filtered_df["family_member_name"] = filtered_df["interest"].apply(
        lambda x: re.search(name_pattern, x).group(1) if re.search(name_pattern, x) else None
    )
    filtered_df["family_member_relationship"] = filtered_df["interest"].apply(
        lambda x: re.search(relationship_pattern, x).group(1) if re.search(relationship_pattern, x) else None
    )
    filtered_df["family_member_role"] = filtered_df["interest"].apply(
        lambda x: re.search(role_pattern, x).group(1) if re.search(role_pattern, x) else None
    )
    
    filtered_df["family_member_paid_by_mp_or_parliament"] = False
    filtered_df.loc[
        filtered_df["category_name"] == "9. Family members employed and paid from parliamentary expenses", 
...
def validate_columns(df):
    ...
    expected_cols = [
        'member_id', 'category_id', 'category_name', 'sort_order', 'interest_id', 
        'unique_interest_id', 'interest', 'created_when', 'last_amended_when', 
        'deleted_when', 'is_correction', 'is_child_interest', 'parent_interest', 
        'contains_time_periods_and_prepositions', 'contains_debt_synonym', 
        'split_amounts', 'max_amount_currency', 'max_amount', 'filtered_amounts', 
        'has_multiple_fullstops', 'extracted_amounts', 'split_currencies', 
        'edited_interest', 'donor_name', 'donor_type', 'donor_address', 
        'companies_house_id', 'family_member_relationship', 
        'family_member_paid_by_mp_or_parliament', 'family_member_name', 
        'family_member_lobbies', 'family_member_role', 'employer_name', 'mp_role'
    ]
...
def make_dates_aware(value):
    ...
    if value is not None and not pd.isna(value):
        if timezone.is_naive(value):
            return timezone.make_aware(value, timezone.utc)  # Convert naive to aware in UTC
        else:
            return value.astimezone(timezone.utc)  # Convert aware datetime to UTC if it's already aware
    return None  
    ...
    try:
        with transaction.atomic():
            for idx, tup in enumerate(tuples):
                ...
                instance_data["member_of_parliament"] = members_dict.get(str(instance_data["member_of_parliament"]))

                # make dates timezone aware
                instance_data["date_created"] = make_dates_aware(instance_data["date_created"])
                instance_data["date_last_amended"] = make_dates_aware(instance_data["date_last_amended"])
                instance_data["date_deleted"] = make_dates_aware(instance_data["date_deleted"])

                # Replace interest_amount NaN's with None to accomodate db field requirements
                instance_data["interest_amount"] = None if pd.isna(instance_data["interest_amount"]) else instance_data["interest_amount"]
                ...
                if len(batch) == batch_size or (current_batch_number == n_batches and len(batch) == last_batch_size):
                    try:
        ...
        if errors:
            records_created = 0
            raise Exception("Errors occurred during bulk save, all transactions rolled back.")
            
    except Exception as global_error:
        # Collect global errors outside the transaction block
        global_error_message = f"{type(global_error).__name__} during transaction: {str(global_error)}"
...
def check_and_adjust_column_types(dataframe: pd.DataFrame, model: models.Model):
    ...
    if mismatched_columns:
        ...
        for col, df_t, expected_t in mismatched_columns:
            print(f"  Column '{col}' has dtype '{df_t}', but expected '{expected_t}' based on model field.")
        
        # Prompt user to adjust the types
        adjust = ''
        while adjust not in ['y', 'n']:
            adjust = input("\nWould you like to automatically adjust the column types to match the model? (y/n): ").strip().lower()
    ...
    for field in RegisteredInterest._meta.fields:
        ...
        try:
            if field_name in df.columns and field_type == "CharField":
                ...
                df[field_name] = df[field_name].where(~exceeds_max_length_mask, df[field_name].str[:field_max_len])
...
def extract_currencies_and_amounts(df):
    ...
    data_w_extractable_amts = df[df["category_name"].isin(EXTRACTABLE_AMOUNT_CATEGORIES)].copy()
    
    data_w_extractable_amts = remove_space_in_registration_numbers(data_w_extractable_amts)
    data_w_extractable_amts = convert_abbreviated_numbers_to_numbers(data_w_extractable_amts,"edited_interest")

    # extract currency and amounts from edited_interest – pattern also returns some normal/other numbers
    data_w_extractable_amts['extracted_amounts'] = (
        data_w_extractable_amts['edited_interest'].apply(lambda x: re.findall(EXTRACT_MONEY_PATTERN, x))
    )

    # filter extracted_amounts for genuine currencies and amounts
    data_w_extractable_amts["filtered_amounts"] = data_w_extractable_amts["extracted_amounts"].apply(filter_currency)

    # check if filtered amounts contains more than one full stop (implies not real amount e.g. date) and filter out if true 
    data_w_extractable_amts['has_multiple_fullstops'] = data_w_extractable_amts['filtered_amounts'].apply(has_multiple_fullstops)
    if data_w_extractable_amts['has_multiple_fullstops'].any():
        print("Some amounts have multiple full stops, implying non-amounts are still present; check this.")
        data_w_extractable_amts = data_w_extractable_amts[data_w_extractable_amts['has_multiple_fullstops'] == True]  # noqa: E712
    ...
    data_w_extractable_amts = extract_max_amount_with_currency(data_w_extractable_amts,"filtered_amounts")
    ...
    merged_df = df.merge(data_w_extractable_amts[merge_cols], on='unique_interest_id', how='left')
    return merged_df
