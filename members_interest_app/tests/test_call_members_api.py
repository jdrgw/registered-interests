...
class FetchDataTestCase(TestCase):
    ...
    @patch(
        "members_interest_app.utils.call_members_api.range", return_value=range(0, 10)
    )
    def test_fetch_members_data(self, mock_range, mock_sleep):
        # Prepare mock responses
        sample_response = {"key1": "value1", "key2": "value2"}
        for i in range(0, 10):  # testing with 10 mock requests
            responses.add(
                responses.GET,
                f"https://members-api.parliament.uk/api/Members/{i}",
        ...
        test_file_path = os.path.join(
            settings.BASE_DIR, "data", "members_data", "raw_data", test_file_name
        )
        ...
        with patch(
            "members_interest_app.utils.call_members_api.os.path.join",
        ...
        self.assertTrue(
            file_exists, f"The JSON file was not created at {test_file_path}"
        )
