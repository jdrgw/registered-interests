...
class TestExtractApiIds(TestCase):
    ...
    def test_no_valid_links_array(self, mock_print):
        ...
        mock_print.assert_any_call("No valid 'links' array in object: {'value': [], 'links': None}")
        mock_print.assert_any_call("No valid 'links' array in object: {'value': [], 'links': 'not_a_list'}")
        mock_print.assert_any_call("No valid 'links' array in object: {'value': [], 'links': []}")
    ...
    def test_no_capture_group(self, mock_print):
        ...
        self.assertTrue(mock_print.called)
        mock_print.assert_any_call("no match for [{'rel': 'self', 'href': '/OtherPath/1/NoMatch', 'method': 'GET'}] when searching for in-file api_ids")
        mock_print.assert_any_call("no match for [{'rel': 'self', 'href': '/OtherPath/2/NoMatch', 'method': 'GET'}] when searching for in-file api_ids")
...
class TestCallRegisteredInterests(TestCase):
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("members_interest_app.utils.call_registered_interests.extract_api_ids_from_file")
    ...
    def test_append_new_data_to_file(self, mock_api_ids, mock_request, mock_extracted_ids, mock_write):
        """
        ...
        mock_api_ids.return_value.order_by.return_value.values_list.return_value = ["1","2"]
        db_api_ids = mock_api_ids.return_value.order_by.return_value.values_list.return_value
        ...
        responses = [
            {
                ...
                "links": [
                    {
                        "rel": "self",
                        "href": f"/Members/{i}/Interests",
                        "method": "GET"
                    }
                ]
            } for i in db_api_ids if str(i) not in extracted_ids
        ]
    ...
    @patch("members_interest_app.utils.call_registered_interests.requests.Session.get")
    @patch("members_interest_app.models.MemberOfParliament.objects.filter")
    def test_server_error(self, mock_api_ids, mock_request, mock_extracted_ids, mock_write, mock_print, mock_sleep):
    ...
    @patch("members_interest_app.utils.call_registered_interests.time.sleep")
    @patch("members_interest_app.utils.call_registered_interests.extract_api_ids_from_file")
    def test_file_path_passed_to_extract_ids_func(self, mock_extract_ids, mock_sleep, mock_open):
        # file_path = MagicMock()
